...
const SpHeader = (props) => {
  ...
  const [isFavouriteProvider, setisFavouriteProvider] = useState(false);
  const history = useHistory();

  useEffect(() => {
    checkIsInFavouriteList(props.businessid);
    loadData().then((res) => {
  ...
  const checkIsInFavouriteList = (providerId) => {
    if (providerId !== null && providerId !== "") {
      HTTP.post(
        API.checkFavoriteProvider,
        ...
        if (res && res.status && res.status === 200) {
          if (res.data) {
            setisFavouriteProvider(res.data);
          }
        }
      });
    }
  ...
  const AddToFavourite = (providerId = null) => {
    if (providerId !== null && providerId !== "") {
      HTTP.post(
        API.addToFavouriteProvider,
              ...
              <div className="mr15">
                <div className="w128">
                  <img
                    ...
                    src={businessdetails && businessdetails.logoimage ? API.imageurl + businessdetails.logoimage : Img.businessPlaceholder.default}
                    alt=""
                    srcset=""
                  />
                </div>
            ...
            <div className="d-flex align-items-center flex-wrap justify-content-end mobJustifyContentStart">
              <CmnButton
                onClick={(e) => {
                  GLocalStorage.Add("c-task-tp", {
                    isb: false,
                    ic: businessdetails.workingcategories._id,
                    ib: businessdetails._id,
                  });
                  history.push({
                    pathname: "/tasks/task-details",
                    state: {
                      categoryid: businessdetails.workingcategories._id,
                    },
                  });
                }}
                type="square"
              ...
              {isFavouriteProvider && (
                <CmnButton
                  ...
                  onClick={(e) => {
                    ConfirmModal02.show(
                      "Are you sure to remove this provider from favourite list?",
                      () => {
                        removeFavouriteProvider(businessdetails._id);
                      }
                    );
                  }}
                  className="btn-link ml10"
                />
              )}
              {!isFavouriteProvider && (
                <CmnButton
                  ...
                  text="Favourite"
                  onClick={(e) => {
                    AddToFavourite(businessdetails._id);
                  }}
...
const ReviewListings = (props) => {
  ...
  const loadData = async () => {
    ...
    try {
      await HTTP.get(
        API.getBusinessReviews + props.businessid + "?pagesize=1&page=1",
        false,
        Auth.getToken()
      ).then((res) => {
